      ...
      <div className="glass-panel rounded-3xl overflow-hidden">
        ...
        <div className="divide-y divide-[var(--border)]">
          {leaders.map((leader, index) => (
            <div key={leader.id} className="grid grid-cols-12 gap-4 p-5 items-center hover:bg-black/5 dark:hover:bg-white/5 transition-colors">
              ...
              <div className="col-span-6 flex items-center gap-4">
                <img src={leader.photoURL || `https://api.dicebear.com/7.x/avataaars/svg?seed=${leader.uid}`} alt={leader.displayName} className="w-12 h-12 rounded-full border border-[var(--border)] shadow-sm" />
                <div>
                  <div className="font-bold text-base flex items-center gap-1">
                    {leader.displayName}
                    {leader.isVerified && <BadgeCheck size={16} className="text-blue-500" />}
                    {leader.isShana && <Flame size={16} className="text-orange-500" />}
                  </div>
